...
describe('buildRedirectUrl', () => {
  it('should redirect to /edit by default when hash is empty', () => {
    expect(buildRedirectUrl(mockLocation('https://example.com/'))).toBe('/edit');
  });

  it('should preserve search params', () => {
    expect(buildRedirectUrl(mockLocation('https://example.com/?utm_source=github'))).toBe(
      '/edit?utm_source=github'
    );
  });

  it('should extract route and fragment from old hash format', () => {
    expect(buildRedirectUrl(mockLocation('https://example.com/#/edit/pako:abc123'))).toBe(
      '/edit#pako:abc123'
    );
  });
  ...
  it('should place search params before the hash fragment', () => {
    expect(
      buildRedirectUrl(mockLocation('https://example.com/?utm_source=twitter#/edit/pako:abc123'))
    ).toBe('/edit?utm_source=twitter#pako:abc123');
  });

  it('should handle hash with view route', () => {
    expect(buildRedirectUrl(mockLocation('https://example.com/#/view/pako:xyz'))).toBe(
      '/view#pako:xyz'
    );
  });

  it('should default to edit when hash has no route', () => {
    expect(buildRedirectUrl(mockLocation('https://example.com/#somethingelse'))).toBe('/edit');
  });
  ...
  it('should handle multiple search params with hash', () => {
    expect(
      buildRedirectUrl(
        mockLocation(
          'https://example.com/?utm_source=gh&utm_medium=link&utm_campaign=test#/edit/pako:data'
        )
      )
