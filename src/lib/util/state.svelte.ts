...
const urlParseFailedState = `flowchart TD
    ...
    C -->|Yes| D(The diagram data looks valid but could not be loaded.<br/>The link may be from an incompatible version.)
...
// Optional observer notified after every persisted change to the active
// document. The tabs layer uses this to mirror the live document into the
// active tab's slot without this module needing to know tabs exist.
let persistListener: ((snapshot: State) => void) | undefined;
export const setPersistListener = (fn: (snapshot: State) => void): void => {
  persistListener = fn;
};
